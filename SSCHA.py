...
#HERE i load the dynamical matrices, i execute some useful commands and I generate the ensamble I need
...
TRAINED_FLAG = False
GENERATE_ENS = True #generates the displaced configurations
HESSIAN      = False
PRETRAINED   = False
LOCAL        = True

CALCULATOR   = "VASP"   #"ESPRESSO"

PATH = "/projects/academic/ezurek/francesco/"
...
RELAX_FLAG = False
static_bulk_modulus = 10
target_press = 450
fix_cell_shape = False
...
# conf_table_gamma (double array with two elements [[1, 12.0], [index of configuration, gamma]] )table of the configurations to do machine learning on
...
while POPULATION < POP_MAX:
   ...
   #####################################################################################
    ...
    if CALCULATOR == "ESPRESSO": fc.Generate_SCF(POPULATION,ENS_FOLDER,PREFIX,typical_espresso_header)
    elif CALCULATOR == "VASP": saved_ordering = fc.Generate_POSCAR(POPULATION,ENS_FOLDER,PREFIX)
    ##############################################################################################
    ...
    #LOADS AND SUBMIT JOBS ON THE CLUSTER
    if GENERATE_ENS == True:
        if CALCULATOR == "ESPRESSO": fc.Send_to_Folders(PREFIX,LOCAL,PATH,folder,ADDRESS,conf_table_gamma,MAX,marker)
        if CALCULATOR == "VASP": fc.Send_to_Folders_VASP(PREFIX,LOCAL,PATH,folder,ADDRESS,conf_table_gamma,MAX,marker)
        print("DONE")
    #################################################################################################
    
    print("going into queue and resubmit")
    #################################################################################################
    if CALCULATOR == "ESPRESSO": fc.Queue_and_resubmit(PREFIX,ADDRESS,PATH,folder,TIMER,LOCAL,conf_table_gamma,MAX,marker)
    if CALCULATOR == "VASP": fc.Queue_and_resubmit_VASP(PREFIX,ADDRESS,PATH,folder,TIMER,LOCAL,conf_table_gamma,MAX,marker)
    #######################################################################################################################################################
    ...
    ##############################################################################################################################################################################
        fc.Train_submit(POPULATION,ADDRESS,PATH,folder,MLIP_PATH,LOCAL,conf_table_gamma)
    ...
    if (grad_for > 0.002 or grad_pos > 0.001) and POPULATION == POP_MAX -1:
        POP_MAX = POP_MAX +1
    ...
    ##################################################################################################################################################################################################
    if RELAX_FLAG == True and grad_for < 0.0001 and grad_pos < 0.0001:
        fix_volume = False
